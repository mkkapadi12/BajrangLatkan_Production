  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {workers.map(worker => (
          <div key={worker._id || worker.id} className="bg-white rounded-lg shadow-md p-4 hover:shadow-xl transition-shadow">
            ...
            <div className="mt-4">
              <p className="text-gray-600">Skills: {worker.skills.join(', ')}</p>
              <p className="text-gray-600 mt-2">Experience: {worker.experience} years</p>
            </div>
            <div className="mt-4 flex justify-between items-center">
              <span className="text-purple-600 font-bold">₹{worker.salary}/month</span>
              <span className={`px-2 py-1 rounded ${worker.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {worker.status}
              </span>
            </div>
          </div>
